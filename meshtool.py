...
def findBed(lines):
    ...
    S3D = False
    # This goes through and tries to check for s3d's bed size. if not, assumes that its a slic3r variant
    for line in lines:
...
def fillGrid(bounds, lines):
    ...
    if Points == True:
        ...
        gridNew = 'M557 X{}:{} Y{}:{} P{}:{}'.format(
            bounds['X']['min'], bounds['X']['max'], bounds['Y']['min'], bounds['Y']['max'], X_points, Y_points)
    else:
        # Fill in the level command template
        gridNew = 'M557 X{}:{} Y{}:{} S{}'.format(
            bounds['X']['min'], bounds['X']['max'], bounds['Y']['min'], bounds['Y']['max'], probeSpacing)

    # Replace M557 command in GCODE
    linesNew = []
    for line in lines:
        if line.startswith('M557'):
            linesNew.append(re.sub(r'^M557 X\d+:\d+ Y\d+:\d+ S\d+',
                            gridNew, line, flags=re.MULTILINE))
...
if __name__ == '__main__':
    if len(sys.argv) == 2:
        ...
        fname = sys.argv[1]
        probeSpacing=int(sys.argv[2])
        main(fname, probeSpacing)
    else:
        
        print('Error: You need need more arguments! arguments should at least be the path to the file, but your second argument can be the desired probe spacing')
        error() 
